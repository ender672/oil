...
Tests should not leak memory:
  ...
  $ ruby -Ilib:test -e "require 'test_jpeg.rb'; require 'test_png.rb'; loop{ MiniTest.run }"

You can loop over a single test for memory leaks as well:

  $ ruby -Ilib:test -e "require 'test_jpeg.rb'; loop{ MiniTest.run(['-n', 'test_each_in_each']) }" > /dev/null
